...
RUN if [ "$VARIANT" = "android" ]; then \
      ...
      if [ "$TARGETARCH" != "arm64" ]; then \
        /opt/android-sdk/cmdline-tools/latest/bin/sdkmanager --sdk_root=/opt/android-sdk "emulator" ; \
      fi && \
      ...
      echo "[android] toolchain OK: jdk17 + sdk + build-tools 34.0.0 + system-images;android-34;google_apis;${IMG_ARCH} (emulator=$([ "$TARGETARCH" != "arm64" ] && echo yes || echo no))" ; \
    fi
